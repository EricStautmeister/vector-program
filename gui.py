...
class GUI:
    ...
    def __init__(self):
        ...
        self.root.geometry("%dx%d+%d+%d" %
                           (width, height, x_cord, y_cord))  # WINDOW SIZE
        ...
        self.vector_frame.place(
            relx=0.0175, relwidth=0.315, rely=0.03, relheight=0.7)
        self.settings_frame.place(
            relx=0.0175, relwidth=0.315, rely=0.75, relheight=0.22)
        self.action_frame.place(rely=0.75, relheight=0.22,
                                relx=0.35, relwidth=0.632)

        # functions class and vectors
        self.Vectors2d = []
        self.Vectors3d = []
        self.function = ProgramControls(
            self.Mainframe, self.Vectors2d, self.Vectors3d)

        # VECTOR FRAME
        self.vector_name = tk.Label(
            master=self.vector_frame, text="Vectors", font="Helvetica 24")
        self.vector_list_win = tk.Label(
            master=self.vector_frame, anchor="nw", font="helvetica 14")
        self.origin_list_win = tk.Label(
            master=self.vector_frame, anchor="nw", font="helvetica 14")
        ...
        self.x_entry = tk.Entry(master=self.vector_frame,
                                justify="center", font="Helvetica 15")
        self.y_entry = tk.Entry(master=self.vector_frame,
                                justify="center", font="Helvetica 15")
        self.x_origin = tk.Entry(
            master=self.vector_frame, justify="center", font="Helvetica 15")
        self.y_origin = tk.Entry(
            master=self.vector_frame, justify="center", font="Helvetica 15")
        self.z_entry = None
        self.z_origin = None
        ...
        self.vector_name.place(relx=0.02, relwidth=0.96,
                               rely=0.015, relheight=0.05)
        self.vector_list_win.place(
            relx=0.25, relwidth=0.365, rely=0.075, relheight=0.38)
        self.origin_list_win.place(
            relx=0.615, relwidth=0.365, rely=0.075, relheight=0.38)
        self.vector_label.place(relx=0.02, relwidth=0.1,
                                rely=0.075, relheight=0.03)
        self.origin_label.place(relx=0.13, relwidth=0.1,
                                rely=0.075, relheight=0.03)
        ...
        self.x_origin.place(relx=0.13, relwidth=0.1,
                            rely=0.115, relheight=0.08)
        self.y_origin.place(relx=0.13, relwidth=0.1,
                            rely=0.205, relheight=0.08)
        self.enter_btn.place(relx=0.02, relwidth=0.212,
                             rely=0.385, relheight=0.07)
        ...
        self.button_2d.place(relx=0.025, relwidth=0.21875,
                             rely=0.05, relheight=0.35)
        self.button_3d.place(relx=0.26875, relwidth=0.21875,
                             rely=0.05, relheight=0.35)
        self.log.place(relx=0.5125, relwidth=0.21875,
                       rely=0.05, relheight=0.35)
        self.place_holder.place(
            relx=0.75625, relwidth=0.21875, rely=0.05, relheight=0.35)
        self.reset_button.place(relx=0.025, relwidth=0.95,
                                rely=0.45, relheight=0.23)
        self.quit_button.place(rely=0.73, relheight=0.23,
                               relx=0.025, relwidth=0.95)

        # Action Frame
        '''self.toolbar = NavigationToolbar2Tk(self.action_frame, PltFrame.plot_frame)
        self.toolbar.pack()'''
        self.x_axis_min = tk.Entry(
            master=self.action_frame, justify="center", font="Helvetica 15")
        self.x_axis_max = tk.Entry(
            master=self.action_frame, justify="center", font="Helvetica 15")
        self.y_axis_min = tk.Entry(
            master=self.action_frame, justify="center", font="Helvetica 15")
        self.y_axis_max = tk.Entry(
            master=self.action_frame, justify="center", font="Helvetica 15")

        self.x_axis_label = tk.Label(
            master=self.action_frame, text="X axis min/max")
        self.y_axis_label = tk.Label(
            master=self.action_frame, text="Y axis min/max")

        self.x_axis_min.place(relx=0.025, relwidth=0.096875,
                              rely=0.55, relheight=0.35)
        self.x_axis_max.place(
            relx=0.146875, relwidth=0.096875, rely=0.55, relheight=0.35)
        self.y_axis_min.place(
            relx=0.26875, relwidth=0.096875, rely=0.55, relheight=0.35)
        self.y_axis_max.place(
            relx=0.390625, relwidth=0.096875, rely=0.55, relheight=0.35)
        self.x_axis_min.insert(0, "-")
        self.x_axis_max.insert(0, "+")
        self.y_axis_min.insert(0, "-")
        self.y_axis_max.insert(0, "+")

        self.x_axis_label.place(
            relx=0.025, relwidth=0.21875, rely=0.05, relheight=0.35)
        self.y_axis_label.place(
            relx=0.26875, relwidth=0.21875, rely=0.05, relheight=0.35)
    ...
    def make_zs(self):
        if self.z_entry:
            return
        self.z_entry = tk.Entry(master=self.vector_frame,
                                justify="center", font="Helvetica 15")
        self.z_origin = tk.Entry(
            master=self.vector_frame, justify="center", font="Helvetica 15")
        self.z_entry.place(relx=0.02, relwidth=0.1, rely=0.295, relheight=0.08)
        self.z_origin.place(relx=0.13, relwidth=0.1,
                            rely=0.295, relheight=0.08)
...
class ProgramControls:
    ...
    def __init__(self, guiframe, Vectors2d, Vectors3d):
        ...
        self.pltframe.plot_frame.place(
            relx=0.35, rely=0.03, relheight=0.7, relwidth=0.632)
    ...
    def create_vector(self, x_entry, y_entry, x_origin, y_origin, z_entry, z_origin, ):
        self.data = self.get_values(
            x_entry, y_entry, x_origin, y_origin, z_entry, z_origin)
        if self.dim == 2:
