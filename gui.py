...
class GUI:
    ...
    def __init__(self):
        ...
        self.root.geometry(
            "%dx%d+%d+%d" % (width, height, x_cord, y_cord)
        )  # WINDOW SIZE
        ...
        self.calc_mode = tk.OptionMenu(
            self.vector_frame, OPTIONS_VALUE, *self.calc_modes_list
        )
        ...
        self.enter_btn = tk.Button(
            master=self.vector_frame,
            text="Enter",
            font="Helvetica 13",
            command=lambda: self.function.create_vector(
                self.x_entry,
                self.y_entry,
                self.x_origin,
                self.y_origin,
        ...
        # SETTINGS FRAME
        self.button_2d = tk.Button(
            master=self.settings_frame,
            text="2D",
            bg=self.button_bg,
            font="Helvetica 20",
            command=lambda: self.function.make_2d(self.remove_zs()),
        )
        self.button_3d = tk.Button(
            master=self.settings_frame,
            text="3D",
            bg=self.button_bg,
            font="Helvetica 20",
            command=lambda: self.function.make_3d(self.make_zs()),
        )
        self.log = tk.Button(
            master=self.settings_frame,
            text="Log",
            bg=self.button_bg,
            font="Helvetica 20",
            command=self.function.make_log,
        )
        self.place_holder = tk.Button(
            master=self.settings_frame,
            text="Test",
            bg=self.button_bg,
            font="Helvetica 20",
            command=lambda: pass_placeholder(),
        )
...
class ProgramControls:
    ...
    def create_vector(
        ...
        if self.dim == 2:
            self.plot_data = [
                [self.data[0][0], self.data[0][0] + self.data[0][1]],
